/* generated from lib/size-read.js — do not edit by hand */
...
function lookupBrandFit(brand, piece, family, name = "") {
...
function resolveRun({ fitNote = "", reviewFit = null, brand = "", piece = "", family = "clothes", name = "" } = {}) {
 ...
 */
function extractSizeCore(raw, family = "clothes") {
...
function shopifySizeIndex(options = []) {
...
function shopifyVariantOptions(product, family = "clothes") {
...
function shopifyOption1FromHtml(html, family = "clothes") {
  const rawValues = [];
 ...
 */
function normalizeLabel(raw, family = "clothes") {
  const original = clip(raw, 80);
  ...
  if (bare) {
    ...
    if (family === "clothes") {
      ...
      if ((n >= 0 && n <= 18 && (n % 2 === 0 || n === 0)) || value === "00") {
        ...
        return { raw: original, label: `us ${norm}`, system: "us", value: norm, family };
      }
    }
...
function parseUserSize(raw, family = "clothes") {
  if (!raw) return null;
...
function collectOptions(labels, family = "clothes") {
  const seen = new Set();
  const out = [];
 ...
 */
function matchUserSize(extracted = {}, sizes = {}, { brand = "", piece = "", reviewFit = null, name = "" } = {}) {
...
function jsonLdSizeOptions(product, family = "clothes") {
