...
export const FPR_SCHEDULE_2026 = [
  { id: "orientation", when: "Wed Aug 26 · 6:00–8:45 PM", date: "2026-08-26" },
  { id: "unity_day_1", when: "Thu Aug 27 · 5:30–10:30 PM", date: "2026-08-27" },
  { id: "unity_day_2", when: "Fri Aug 28 · 4:30–11:00 PM", date: "2026-08-28" },
  { id: "sisterhood_day", when: "Sat Aug 29 · 9:20 AM–8:45 PM", date: "2026-08-29" },
  { id: "philanthropy_day", when: "Sun Aug 30 · 9:20 AM–7:00 PM", date: "2026-08-30" },
  { id: "preference", when: "Mon Aug 31 · 6:00–11:30 PM", date: "2026-08-31" },
  { id: "bid_day", when: "Tue Sep 1 · 6:00 PM", date: "2026-09-01" },
];

/** Rounds she can still dress for. Berkeley is UTC-7 in late august. */
function todayInBerkeley(now = new Date()) {
  return new Date(now.getTime() - 7 * 60 * 60 * 1000).toISOString().slice(0, 10);
}

/**
 * The next round she can still wear something to.
 *
 * The fallback when a scan comes back without a day attached. Naming a real
 * night is the whole point of the answer — "recruitment" tells her nothing she
 * did not already know, and she is holding the dress right now.
 */
export function nextRoundId(now = new Date()) {
  const today = todayInBerkeley(now);
  const ahead = FPR_SCHEDULE_2026.find((r) => r.date >= today && r.id !== "orientation" && r.id !== "bid_day");
  // Past the last round the honest answer is the one they dress up most for.
  return ahead?.id || "preference";
}

/**
 * Turn whatever the model said into a single night she can act on.
 *
 * "unity_day" is not an answer — thursday and friday are two different outfits
 * and telling her "unity" leaves her with the same question she arrived with.
 */
export function resolveRoundId(roundId, now = new Date()) {
  const id = String(roundId || "").trim();
  if (id && id !== "unity_day" && id !== "orientation") return id;
  if (id === "unity_day") {
    const today = todayInBerkeley(now);
    const night = ["unity_day_1", "unity_day_2"]
      .map((x) => FPR_SCHEDULE_2026.find((r) => r.id === x))
      .find((r) => r && r.date >= today);
    return night?.id || "unity_day_2";
  }
  return nextRoundId(now);
}
