...
import { mergeGmailSizes, parseGmailMessage } from "./gmail-parse.js";
import { sizeMap } from "./profile.js";
import { upsertClosetFromGmail } from "./store.js";
...
export async function syncGmail(userId, account) {
  const access = await freshAccessToken(account);
  const messages = await searchGmail(access, { max: 32 });
  ...
  for (const m of messages) {
    ...
    let body = "";
    if (["order", "return", "sizing"].includes(signal_kind)) {
      try {
        const full = await fetchGmailMessage(access, m.gmail_id);
        body = full?.body || "";
      } catch {
        body = "";
      }
    }
    const parsed = parseGmailMessage({ ...m, brand, signal_kind, body });
    ...
    rows.push({
      ...
      signal_kind: parsed.signal_kind || signal_kind,
      brand: parsed.brand || brand,
      item_name: parsed.items[0]?.item || "",
      size: parsed.items[0]?.size || parsed.sizes[0] || "",
      parsed: parsed.items.length ? { items: parsed.items, sizes: parsed.sizes, summary: parsed.summary } : null,
      raw: m,
    });
  }
  ...
  if (parsedItems.length) {
    await upsertClosetFromGmail(userId, parsedItems);
    const profileRes = await sbAdmin(rest("profiles", `id=eq.${userId}&select=sizes`));
    const profile = one(profileRes.data);
    const merged = mergeGmailSizes(sizeMap(profile?.sizes || {}), parsedItems);
  ...
  return { count: rows.length, closet: parsedItems.length };
}
...
export async function listGmailSignals(userId, { limit = 30 } = {}) {
  const res = await sbAdmin(
    rest(
      "gmail_signals",
      `user_id=eq.${userId}&order=sent_at.desc.nullslast&limit=${limit}&select=id,subject,from_addr,snippet,sent_at,signal_kind,brand,item_name,size,parsed`
    ),
    { method: "GET" }
  );
  return Array.isArray(res.data) ? res.data : [];
