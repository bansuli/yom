...
/** Full message for receipt parsing (subject + plain/html body). */
export async function fetchGmailMessage(accessToken, id) {
  const msg = await googleFetch(accessToken, `${GMAIL}/users/me/messages/${id}?format=full`);
  if (!msg.ok) return null;
  const normalized = normalizeGmailMessage(msg.data);
  const { decodeGmailBody } = await import("./gmail-parse.js");
  return { ...normalized, body: decodeGmailBody(msg.data.payload) };
}
