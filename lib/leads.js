...
export async function touchScanVisitor(input = {}) {
  ...
  if (supabaseConfigured()) {
    ...
    if (found?.id) {
      ...
      // Upsert on the device id. The client retries a visit that never got a
      // reply, and when the first one had in fact landed, a plain insert made
      // the retry a duplicate-key failure — a fault report for a visit that was
      // saved correctly the first time.
      const inserted = await sbAdmin(rest("scan_visitors", "on_conflict=anon_id"), {
        method: "POST",
        prefer: "resolution=merge-duplicates,return=representation",
        body: {
          ...row,
