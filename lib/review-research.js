 ...
 * TikTok uses lib/tiktok-research.js for multi-query discovery + verification.
...
import {
  researchTikTokEvidence,
  tiktokHighlights,
  tiktokReviewLine,
} from "./tiktok-research.js";
...
function mergeTikTokBrief(brief, tiktokEvidence) {
  if (!tiktokEvidence) return brief;
  if (!brief) {
    const created = {
      summary: tiktokEvidence.consensus || "",
      fit: tiktokEvidence.fit || "unknown",
      fit_note: tiktokReviewLine(tiktokEvidence),
      size_shift: null,
      quality: "",
      occasions: "",
      sentiment: "unknown",
      channels: ["tiktok"],
      highlights: tiktokHighlights(tiktokEvidence),
      coverage: { searched: SEARCHED, found: ["tiktok"] },
      tiktok: tiktokEvidence,
    };
    created.regret_delta = reviewRegretDelta(created);
    return created;
  }
  const ttHighlights = tiktokHighlights(tiktokEvidence);
  const mergedHighlights = [...ttHighlights, ...(brief.highlights || [])].slice(0, 6);
  const channels = [...brief.channels];
  if (!channels.includes("tiktok")) channels.unshift("tiktok");
  const fit =
    brief.fit && brief.fit !== "unknown"
      ? brief.fit
      : tiktokEvidence.fit && tiktokEvidence.fit !== "unknown"
        ? tiktokEvidence.fit
        : brief.fit;
  const summary =
    brief.summary ||
    tiktokEvidence.consensus ||
    (tiktokEvidence.video_count ? `${tiktokEvidence.video_count} verified tiktok try-ons` : "");
  return {
    ...brief,
    summary: clip(summary, 280).toLowerCase(),
    fit,
    fit_note: brief.fit_note || tiktokReviewLine(tiktokEvidence),
    channels: channels.slice(0, 8),
    highlights: mergedHighlights,
    tiktok: tiktokEvidence,
    coverage: {
      searched: SEARCHED,
      found: channels.slice(0, 8),
    },
    regret_delta: reviewRegretDelta({ ...brief, tiktok: tiktokEvidence, fit, highlights: mergedHighlights }),
  };
}

export function reviewRegretDelta(brief) {
  if (!brief?.channels?.length && !brief?.highlights?.length && !brief?.tiktok?.video_count) return 0;
  let n = 0;
  ...
  const tiktokBlob = `${brief.tiktok?.consensus || ""} ${(brief.tiktok?.sizing_signals || [])
    .map((s) => s.text)
    .join(" ")} ${(brief.tiktok?.fit_claims || []).map((f) => f.claim).join(" ")}`;
  if (brief.tiktok?.fit === "runs small") n += 6;
  if (brief.tiktok?.fit === "runs large") n += 3;
  if (/size down|runs large|too big|waist gap/i.test(tiktokBlob)) n += 4;
  if (/size up|runs small|tight/i.test(tiktokBlob)) n += 6;
  const blob = `${brief.fit_note || ""} ${brief.quality || ""} ${brief.summary || ""} ${tiktokBlob} ${(brief.highlights || [])
    .map((h) => h.text)
    .join(" ")}`;
...
export function reviewLine(brief) {
  if (!brief) return "";
  const tiktok = tiktokReviewLine(brief.tiktok);
  if (tiktok) return clip(tiktok, 160);
...
export function formatReviewsForPrompt(brief) {
  if (!brief?.summary && !brief?.highlights?.length && !brief?.tiktok?.consensus) return "";
  const tiktokBits = [];
  if (brief.tiktok?.consensus) tiktokBits.push(`tiktok consensus: ${brief.tiktok.consensus}`);
  if (brief.tiktok?.fit && brief.tiktok.fit !== "unknown") tiktokBits.push(`tiktok fit: ${brief.tiktok.fit}`);
  (brief.tiktok?.sizing_signals || []).slice(0, 4).forEach((s) => {
    tiktokBits.push(
      `- [tiktok] ${[s.height, s.size_worn ? `size ${s.size_worn}` : "", s.text].filter(Boolean).join(", ")}${s.url ? ` (${s.url})` : ""}`
    );
  });
  return [
...
export async function researchProductReviews({
  ...
  const [reddit, web, brand, hauls, creators, tiktokEvidence] = await Promise.all([
  ...
  const gathered = [
    ...
    ...hauls,
    ...creators,
    ...tiktokHighlights(tiktokEvidence),
    ...pageHits,
  ].filter(Boolean);
  ...
  return mergeTikTokBrief(compactBrief(raw, gathered), tiktokEvidence);
}
