 ...
 * Pulls reddit, amazon, tiktok, the brand site, and clothing/sizing forums.
...
const FORUM_HOSTS =
  /styleforum\.net|thefashionforum\.com|superthread\.com|thefashionspot\.com|makeupalley\.com|trustpilot\.com|sitejabber\.com|thecut\.com|refinery29\.com|whowhatwear\.com|reddit\.com/i;

const CHANNELS = new Set(["reddit", "amazon", "tiktok", "youtube", "brand", "forums", "web"]);
...
export function shouldResearchReviews(product = {}, sourceUrl = "") {
  ...
  const name = String(product.name || product.guess || "").trim();
  if (/^(this piece|unknown|unidentified|item|garment|clothing item|this look)$/i.test(name)) {
    return Boolean(brand);
  }
  if (brand && name) return true;
  const tokens = name.split(/\s+/).filter((w) => w.length > 2 && !/^(the|and|with|for|from|this|that)$/i.test(w));
...
async function searchReddit(query) {
  if (!query) return [];
  const q = encodeURIComponent(`${query} (review OR sizing OR "runs small" OR "runs large" OR "true to size" OR fit)`);
  const urls = [
...
async function searchDuckDuckGo(query) {
  if (!query) return [];
  const searches = [
    `${query} review (reddit OR amazon OR tiktok)`,
    `${query} ("try on" OR haul OR grwm OR "get ready with me") (tiktok OR youtube)`,
...
async function oembedTitle(url) {
  const endpoint = /tiktok\.com/i.test(url)
...
async function searchHaulVideos(query) {
  ...
  const html = await fetchText(
    `https://html.duckduckgo.com/html/?q=${encodeURIComponent(`${query} tiktok youtube "try on" OR haul OR grwm OR "try-on haul"`)}`,
    { timeout: 5500 }
  );
  const urls = [];
  const seen = new Set();
  for (const m of html.matchAll(/href="([^"]+)"/gi)) {
    ...
    const clean = tt || (yt ? `https://www.youtube.com/watch?v=${yt}` : "");
...
function reviewPrompt(product, sourceUrl, query, snippets) {
  const blob = (snippets || [])
    ...
    .join("\n");
  return `Research real shopper reviews for this clothing product. Cover ALL of these that exist:
1. reddit (fashion + sizing subs: petite, plus, tall, xxs, brand communities)
2. amazon reviews
3. tiktok videos — try-on hauls, GRWMs, "get ready with me", stitching, comments under those videos
4. youtube try-on hauls / GRWMs (spoken fit notes in titles and captions)
5. the brand's own website reviews
6. clothing and sizing forums (styleforum, fashion forum, superthread, makeup alley when relevant)
...
Return ONLY json:
{
  "summary": "1-2 lowercase sentences of what shoppers actually say",
  "fit": "runs small|true to size|runs large|mixed|unknown",
  "fit_note": "one lowercase line on sizing, or empty",
  "quality": "one lowercase line on fabric/quality, or empty",
  "sentiment": "positive|mixed|negative|unknown",
  "channels": ["reddit","amazon","tiktok","youtube","brand","forums"],
  "highlights": [{"channel":"reddit","text":"...","url":"https://..."}]
}

Rules:
- never invent a quote, rating, url, or channel
- only list channels you actually found
- prefer fit, fabric, pilling, shrinking, returns over "cute dress"
- max 5 highlights, lowercase
      ...
      messages: [
        {
          role: "system",
          content:
            "you research clothing reviews across reddit, amazon, tiktok try-on hauls and GRWMs, youtube hauls, brand sites, and sizing forums. never invent sources.",
        },
...
function compactBrief(raw, gathered) {
  ...
  const addCh = (c) => {
    if (c === "web") c = "forums";
    if (!CHANNELS.has(c) || c === "web") return;
  ...
  return {
    ...
    sentiment: /^(positive|mixed|negative|unknown)$/.test(String(raw?.sentiment || ""))
      ...
      : "unknown",
    channels: channels.filter((c) => c !== "web").slice(0, 6),
...
export async function researchProductReviews({
  ...
  const [reddit, web, brand, hauls] = await Promise.all([
    withTimeout(searchReddit(query), Math.min(6500, left()), []),
    withTimeout(searchDuckDuckGo(query), Math.min(6500, left()), []),
    sourceUrl ? withTimeout(scrapeBrandPage(sourceUrl), Math.min(6500, left()), []) : [],
    withTimeout(searchHaulVideos(query), Math.min(7000, left()), []),
  ]);

  const gathered = [...reddit, ...web, ...brand, ...hauls, ...pageHits].filter(Boolean);
