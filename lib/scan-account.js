...
import { assembleAccount, ingestOnboarding, seedFounderIfNeeded } from "./profile.js";
import {
  createAuthUser,
  findAuthUserByEmail,
  generateMagicLink,
...
/*
 * A password nobody will ever type: the account is reached through Google.
 *
 * Kept under bcrypt's 72-byte limit. Two UUIDs came to 76 characters, and
 * Supabase answers anything longer with a bare 500 and "unexpected_failure",
 * which is indistinguishable from the database being broken — so every signup
 * failed, and said nothing about why.
 *
 * 24 random bytes is 192 bits of entropy in 32 base64url characters; the
 * suffix satisfies any upper/lower/digit/symbol rule.
 */
...
export async function findOrCreateUserFromGoogle(googleUser = {}, extra = {}) {
  ...
  // Founders carry a curated profile; everyone else just needs the row.
  await seedFounderIfNeeded(authUser);
