...
import { RUSH_STYLIST_ADDON, STYLIST_VOICE } from "./stylist.js";

const TRAIT_LEANS = {
  impulse: "buys things they never wear",
  nothing: "always thinks they have nothing to wear",
  panic: "panics before trips and events",
  decide: "spends forever deciding what to buy",
};

const PRE_BUY_LEANS = {
  friends: "asks friends before buying",
  feed: "scrolls pinterest or tiktok before buying",
  research: "compares many sites before buying",
  wing: "wings it",
};

function formatShopperBlock(context = {}) {
  const traitKey = String(context.trait || "").trim();
  const preKey = String(context.preBuy || "").trim();
  const trait = TRAIT_LEANS[traitKey] || TRAIT_LEANS[traitKey.replace(/\s+/g, "_")] || "";
  const pre = PRE_BUY_LEANS[preKey] || "";
  const read = String(context.read || "").trim();
  const closet = String(context.closet_note || "").trim();
  const lines = [];
  if (read) lines.push(`onboarding_read: ${read.slice(0, 400)}`);
  if (trait) lines.push(`shopper_lean: ${trait}`);
  else if (traitKey) lines.push(`shopper_lean: ${traitKey}`);
  if (pre) lines.push(`how_they_buy: ${pre}`);
  else if (preKey) lines.push(`how_they_buy: ${preKey}`);
  if (closet) lines.push(`onboarding_closet: ${closet.slice(0, 400)}`);
  if (!lines.length) return null;
  lines.push(
    "use onboarding as a lens on THIS piece. one specific behavior, max. do not recap their personality. do not dump the onboarding read back as a paragraph."
  );
  return lines.join("\n");
}
...
- pieces[] = per-item feedback. each feedback line notes how that piece reads for the round.`
    : `- score = how well the FULL LOOK works for their calendar, closet, and the actual occasion (formality, shoes, hem, fabric, cohesion).
- why_it_works = what makes the whole outfit work for THIS person (not just in a vacuum).
- change = one tweak for the whole look (usually shoes, bag, or layer).
- pieces[] = per-item feedback on each visible garment.`
}
...
NEVER write:
- em-dashes, en-dashes used as a pause, or " - " as a pause. use a period or a comma. "skip. you can't walk a wedding in these" not "skip — you can't walk"
...
body: "cream column midi, viscose. hem looks walkable, straps wide enough to sit. not formal enough for preference. don't make it do both jobs."`
    : `title: "buy for the wedding"
body: "navy column midi, matte fabric. hem looks walkable, straps wide enough to sit through dinner. you don't own this silhouette in gmail."`
}
...
return ONLY compact json:
{
  ...
  "verdict": {
    ...
    "score": number,
    "round": "unity_day_1" | "unity_day_2" | "sisterhood_day" | "philanthropy_day" | "preference" | "bid_day",
    "why_it_works": string,
    "change": string,
    "berkeley": string,
    "spotting": string,
...
- round is REQUIRED for anything wearable. never null. she is standing in the store holding it and the day is the answer she came for. if it suits several rounds, name the ONE it is best on. if it suits none well, name the one it is least wrong on and say why in change.
- never return a generic "unity_day". unity day 1 (thu 27, outdoor evening) and unity day 2 (fri 28, outdoor evening) are TWO different outfits — pick unity_day_1 or unity_day_2, never both.
- why_it_works must justify the day you picked, not the piece in the abstract.
