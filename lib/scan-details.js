...
export function mergeDetailsIntoVerdict(product, verdict = {}, styleNotes = null) {
  ...
  const existing = String(verdict.body || "").toLowerCase();
  const missing = details.filter((d) => !existing.includes(d.text.slice(0, 24)));
  const bodyExtra = missing.map((d) => d.text).join(" ");
  return {
    ...verdict,
    details,
    body: [verdict.body, bodyExtra].filter(Boolean).join(" ").trim(),
  };
}
