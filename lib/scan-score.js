...
export function capOutfitShoeVerdict(verdict = {}, pieces = []) {
  if (!Array.isArray(pieces) || !pieces.length) return verdict;
  const shoe = pieces.find((p) => p?.slot === "shoes");
  if (!shoe) return verdict;
  const product = { name: shoe.name, category: shoe.category || "shoes", brand: shoe.brand, guess: shoe.name };
  return capRunningTrainerVerdict(product, verdict);
}
