 ...
 */
export async function logServerError({ kind = "server_error", message = "", status, path, email, anon_id, detail } = {}) {
  ...
  try {
    await sbAdmin(rest("app_errors"), {
      ...
      body: {
        ...
        // Who it happened to, when she has not given an email yet. Without it
        // /admin counts a fault as affecting nobody.
        anon_id: String(anon_id || "").slice(0, 80) || null,
