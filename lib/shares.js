...
export async function saveScanCheck(input = {}) {
  ...
  const row = {
    ...
    // The photo is the whole point of a check — without it the record says a
    // girl scanned "a top" and nobody can see what she was looking at.
    image_url:
      typeof input.image === "string" && (input.image.startsWith("data:image/") || /^https?:\/\//i.test(input.image))
        ? input.image.slice(0, 180000)
        : null,
  };
  ...
  let check = null;
  if (supabaseConfigured()) {
    let inserted = await sbAdmin(rest("scan_checks"), { method: "POST", body: row });
    // image_url arrived after this table did, so a database that has not had the
    // column added yet rejects the whole row. The photo is worth having; the
    // record of the scan is worth more — keep the scan, drop the photo.
    if (!inserted.ok && row.image_url && /image_url/.test(String(inserted.data?.message || ""))) {
      const { image_url: _noColumn, ...withoutImage } = row;
      inserted = await sbAdmin(rest("scan_checks"), { method: "POST", body: withoutImage });
    }
    if (inserted.ok) {
      check = one(inserted.data);
    } else {
      // Every scan used to vanish here without a word: the insert was ignored
      // and the request answered ok, so nothing anywhere knew the check was
      // gone. Say so, and /admin shows it.
      await logServerError({
        kind: "scan_not_saved",
        message: `Could not save scan check — ${inserted.data?.message || "database rejected it"}`,
        status: inserted.status,
        path: "/api/share",
        email: row.email || "",
        anon_id: row.anon_id || "",
        detail: { has_image: Boolean(row.image_url) },
      });
    }
  }
    ...
    const imageData =
      typeof input.image === "string" && input.image.startsWith("data:image/")
        ? input.image.length <= 1_800_000
          ...
          : ""
        : "";
    const sheet = await sheetAppendScanCheck({
      at: new Date().toISOString(),
      anon_id: row.anon_id,
      email: row.email,
  ...
  // Nowhere kept it. Answering ok here is what let a night of scans disappear.
  if (!check && !sheetOk) {
