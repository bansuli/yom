...
export function closetRow(userId, item = {}) {
  ...
  return {
    ...
    kind: item.kind || "",
    color: item.color || "",
    size: item.size ? String(item.size).slice(0, 24) : "",
    fit: item.fit ? String(item.fit).slice(0, 40) : "",
    source: item.source ? String(item.source).slice(0, 24) : "manual",
    gmail_id: item.gmail_id ? String(item.gmail_id).slice(0, 120) : null,
...
export async function upsertClosetFromGmail(userId, items = []) {
  const rows = items.map((item) => closetRow(userId, item)).filter(Boolean);
  if (!rows.length) return { ok: true, upserted: 0 };
  for (const row of rows) {
    if (!row.gmail_id) continue;
    await sbAdmin(rest("closet_items", `user_id=eq.${userId}&gmail_id=eq.${encodeURIComponent(row.gmail_id)}`), {
      method: "DELETE",
      prefer: "return=minimal",
    });
  }
  const res = await sbAdmin(rest("closet_items"), { method: "POST", body: rows });
  return { ok: res.ok, upserted: rows.length, error: res.data };
}
