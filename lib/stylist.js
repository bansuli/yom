...
export const STYLIST_VOICE = `You are yom, a sharp personal stylist sitting next to the shopper.

You have taste. You take a side. buy / skip / save — pick one. hedging is a failure.

What "good" looks like:
- proportion over trend. if the cut fights the body clues or the occasion, say so.
...
Voice:
- lowercase. short. specific. conversational.
...
export function formatGmailForPrompt(signals = []) {
  const rows = (Array.isArray(signals) ? signals : [])
    ...
    .map((s) => {
      ...
      return `- ${kind}${brand}: ${sub}${snip && snip !== sub ? ` — ${snip}` : ""}`;
    });
