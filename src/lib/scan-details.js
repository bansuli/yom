import { humanizeTake } from "./voice.js";
...
export function enrichScanTake(data) {
  ...
  const details = [];
  if (colorText) details.push({ key: "color", label: "color", text: humanizeTake(colorText.toLowerCase()) });
  if (styleText) details.push({ key: "style", label: "style", text: humanizeTake(styleText.toLowerCase()) });
  if (wearText) details.push({ key: "wear", label: "how to wear", text: humanizeTake(wearText.toLowerCase()) });
  details.push(...keep);
